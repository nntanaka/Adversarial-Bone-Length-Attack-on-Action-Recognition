...
class Processor(IO):
    ...
    def start(self):
        ...
        # test phase
        ...
        elif self.arg.phase == 'collect_data':
            if self.arg.weights is None:
                raise ValueError('Please appoint --weights.')
            self.io.print_log('Model:   {}.'.format(self.arg.model))
            self.io.print_log('Weights: {}.'.format(self.arg.weights))
            ...
            # evaluation
            self.io.print_log('Get the data the model successfully classifies:')
            self.get_correctly_classified_data()
            self.io.print_log('Done.\n')
        elif self.arg.phase == 'adversarial_attack':
            if self.arg.weights is None:
                raise ValueError('Please appoint --weights.')
            self.io.print_log('Model:   {}.'.format(self.arg.model))
            self.io.print_log('Weights: {}.'.format(self.arg.weights))
            ...
            # evaluation
            self.io.print_log('Adversarial Bone Length Attack Start:')
            self.adversarial_bone_length_attack()
            self.io.print_log('Done.\n')
    ...
    def get_parser(add_help=False):
        ...
        # processor
        parser.add_argument('--phase', default='train', help='must be train or test or collect_data or adversarial_attack')
